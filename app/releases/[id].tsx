    ...
    <SafeAreaView edges={["left", "right"]}>
      <ScrollView>
        ...
        <Card size="md" variant="elevated" className="m-1">
          <Text size="2xl">
            {data.name.main} {data.is_ongoing && <Heading>Онгоинг</Heading>}
          </Text>
          <Text>{data.description}</Text>

          <Heading>
            Жанры: <Text>{data.genres.map((v) => v.name).join(", ")}</Text>
          </Heading>
          <Heading>
            Год выхода: <Text>{data.year}</Text>
          </Heading>
        </Card>
          ...
          <Card size="md" variant="elevated" className="m-1">
            {data.episodes.map((v) => (
              <Pressable
                className="m-1"
                key={v.id}
                onPress={() => router.navigate("/player/" + v.id)}
              >
                <Text>
                  {v.ordinal} - {v.name}
                </Text>
                ...
                <Suspense>
                  {SecureStore.getItemAsync(`ep_ct_${v.id}`).then((value) => {
                    if (value) {
                      return (
                        <Text>
                          Продолжить с {secondsToMMSS(parseFloat(value))} сек
                        </Text>
                      );
                    }
                  })}
