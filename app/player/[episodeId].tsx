    ...
    <>
      <Video
        ...
        source={{
          ...
          metadata: {
            title: data.name,
            imageUri: data.preview
              ? ANLIB_API_BASE + data.preview.src
              : undefined,
          },
        }}
        ...
        onFullscreenPlayerDidPresent={async () =>
          await ScreenOrientation.lockAsync(
            ScreenOrientation.OrientationLock.LANDSCAPE,
          )
        }
        onFullscreenPlayerDidDismiss={async () =>
        ...
        chapters={[
          data.opening && {
            title: "op",
            startTime: data.opening.start,
            endTime: data.opening.stop,
          },
          data.ending && {
            title: "end",
            startTime: data.ending.start,
            endTime: data.ending.stop,
          },
        ]}
        ...
        onProgress={async (progress) => {
          if (
            progress.currentTime >= data.opening.start &&
            progress.currentTime <= data.opening.stop
          ) {
            videoRef.current?.seek(data.opening.stop);
          } else if (
            progress.currentTime >= data.ending.start &&
            progress.currentTime <= data.ending.stop
          ) {
            videoRef.current?.seek(data.ending.stop);
          }
          await SecureStore.setItemAsync(
            `ep_ct_${data.id}`,
            progress.currentTime.toFixed(3),
          );
        }}
      />
    </>
